...
const NIGHTLY_QUERY_LIMIT = Number(process.env.NIGHTLY_HUNT_QUERY_LIMIT || 2);
const NIGHTLY_MAX_FALLBACK_PER_QUERY = Number(
  process.env.NIGHTLY_HUNT_MAX_FALLBACK_PER_QUERY || 4
);
const NIGHTLY_TARGET_CREATED_APPLICATIONS = Number(
  process.env.NIGHTLY_HUNT_TARGET_CREATED || 6
);
const NIGHTLY_USE_CRAWLER =
  String(process.env.NIGHTLY_HUNT_USE_CRAWLER || "false").toLowerCase() === "true";

async function sendTelegramSniperAlert(chatId, createdApplications) {
  ...
  const text = [
    ...
    "Open your Kanban board to review and apply.",
  ].join("\n");
