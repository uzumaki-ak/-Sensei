...
const CRAWLER_HTTP_TIMEOUT_MS = Number(process.env.HUNT_CRAWLER_TIMEOUT_MS || 60000);
const CRAWLER_BASE_URL = process.env.HUNT_CRAWLER_URL?.trim();
const CRAWLER_TOKEN = process.env.HUNT_CRAWLER_TOKEN?.trim();
...
async function callCrawler(path, payload) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Crawler HTTP ${response.status}${body ? `: ${body.slice(0, 300)}` : ""}`);
    }
