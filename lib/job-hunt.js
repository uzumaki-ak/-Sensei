...
function isBudgetExceeded(startedAt, budgetMs) {
...
function rotateArray(values, seed) {
  if (!Array.isArray(values) || values.length <= 1) return values;
  const offset = Math.abs(seed || 0) % values.length;
...
export async function runJobHuntForUser(user, options = {}) {
  ...
  const maxTotalMs = toPositiveInt(
    options.maxTotalMs ?? process.env.HUNT_MAX_TOTAL_MS,
    0
  );
  const queryLimit = toPositiveInt(
    options.queryLimit ?? process.env.HUNT_QUERY_LIMIT,
    MAX_QUERIES_PER_USER
  );
  const maxUrls = toPositiveInt(
    options.maxUrls ?? process.env.HUNT_URL_LIMIT,
    MAX_URLS_PER_USER
  );
  const maxFallbackPerQuery = toPositiveInt(
    options.maxFallbackPerQuery ?? process.env.HUNT_MAX_FALLBACK_PER_QUERY,
    MAX_FALLBACK_JOBS_PER_QUERY
  );
  const queryRotationSeed = toPositiveInt(
    options.queryRotationSeed ?? process.env.HUNT_QUERY_ROTATION_SEED,
    getUtcDayOfYear()
  );
  const targetCreatedApplications = toPositiveInt(
    options.targetCreatedApplications ?? process.env.HUNT_TARGET_CREATED_APPLICATIONS,
    6
  );
  const useCrawler = options.useCrawler !== false;
  const useFallback = options.useFallback !== false;
  const startedAt = Date.now();

  const allQueries = await buildDiscoveryQueries(user);
  const queries = rotateArray(allQueries, queryRotationSeed).slice(0, queryLimit);
  ...
  let crawlerAvailable = useCrawler;
  const discoveryStartedAt = Date.now();

  for (const query of queries) {
    if (!crawlerAvailable) break;
    if (isBudgetExceeded(startedAt, maxTotalMs)) {
      log("[Hunt] Hunt runtime budget exceeded before discovery; moving to fallbacks.");
  ...
  for (const url of candidateUrls) {
    if (isBudgetExceeded(startedAt, maxTotalMs)) {
      log("[Hunt] Hunt runtime budget exceeded during scrape phase.");
      break;
    }
    try {
  ...
  for (const query of queries) {
    if (isBudgetExceeded(startedAt, maxTotalMs)) {
      log("[Hunt] Hunt runtime budget exceeded before fallback phase completion.");
      break;
    }
    console.log('[HUNT] Fetching fallback jobs for:', query);
    let fallbackJobs = [];
    try {
      const fallbackFetchLimit = Math.min(
        32,
        Math.max(maxFallbackPerQuery, maxFallbackPerQuery * 3)
      );
    ...
    for (const fallbackJob of fallbackJobs) {
      if (isBudgetExceeded(startedAt, maxTotalMs)) {
        log("[Hunt] Hunt runtime budget exceeded during fallback ingestion.");
        break;
      }
      try {
    ...
    if (totals.createdApplications >= targetCreatedApplications) {
      break;
    }
  }
